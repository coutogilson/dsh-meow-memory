...
- memory_remember.param.project: Required. Project name (must be a concrete project name when level=project); use "global" for globally applicable information; comma-separate several projects, e.g. "dsh,femo".
...
- memory_search.param.project: Filter by project name (comma-separated, OR semantics, e.g. "dsh,femo"; "global" and unlabeled entries are always included).
...
- memory_project.description: Retrieve a project's complete injection block from the memory store (plain text, grouped by subcategory, every entry that is not out of date at once). project is required: which project do you want to see? Calling without it raises an error, so settle on the project name first. Call it when the user brings up a project (femo/meow-memory/meow-eyes/dsh, ...) and asks about its design history, technical decisions, their own past words, or where it stands; also whenever you need the project-wide picture before you answer. Rules: within a group, entries run old -> new by memory timestamp; the todo subcategory prints "Done:" (the 5 most recently completed) followed by "To do list:"; every entry carries its full id, its last-updated timestamp, and its text.
